...
if not 'MAVLINK_DIALECT' in os.environ:
    os.environ['MAVLINK_DIALECT'] = 'autoquad'
...
class mavfile(object):
    ...
    def wait_sys_status(self, blocking=True):              # YOON ADDED
        '''wait for a sys_status'''
        return self.recv_match(type='SYS_STATUS', blocking=False)

    def wait_command_ack(self, blocking=True):              # YOON ADDED
        '''wait for a command_ack'''
        return self.recv_match(type='COMMAND_ACK', blocking=blocking)
